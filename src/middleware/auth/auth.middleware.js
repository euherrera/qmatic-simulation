...
function getUserData(payload) {
  ...
  Object.values(payload).forEach((val) => {
    const valueName = val.toLowerCase();

    if (valueName.includes('email') && !data.email) {
      data.email = val;
    } else if (valueName.includes('first') && !data.first_name) {
      data.first_name = val;
    } else if (valueName.includes('last') && !data.last_name) {
      data.last_name = val;
    }
  });
